...
public class BoardModel {
			...
			for (int j = 0; j < tiles.size(); j++) {
				if (tiles.get(j).getX() == x && tiles.get(j).getY() == y) {
					...
					if (type.equals("brick"))
						t = new ResourceTile(ResourceType.BRICK,
					...
					if (type.equals("wheat"))
						t = new ResourceTile(ResourceType.WHEAT,
								new NumberToken(number));
	...
	/**
	 * checks the port type at the location x,y
	 * @param x
	 * @param y
	 * @param direction
	 * @return - the port type at the location x,y
	 */
	public PortType checkPort(int x, int y, String direction) {
		PortType toReturn = null;

		if ((x == 1 && y == -3 && (direction.equals("SE") || direction
				.equals("SW")))
				|| (x == 1 && y == -2 && (direction.equals("NE") || direction
						.equals("NW")))
				|| (x == 2 && y == -3 && direction.equals("W"))
				|| (x == 2 && y == -3 && direction.equals("E")))
			toReturn = ((PortTile) getHexTileAt(1, -3)).getPortType();

		if ((x == -1 && y == -2 && (direction.equals("SE") || direction
				.equals("SW")))
				|| (x == -1 && y == -1 && (direction.equals("NE") || direction
						.equals("NW")))
				|| (x == 0 && y == -2 && direction.equals("W"))
				|| (x == -2 && y == -1 && direction.equals("E")))
			toReturn = ((PortTile) getHexTileAt(-1, -2)).getPortType();

		if ((x == 3 && y == -3 && (direction.equals("W") || direction
				.equals("SW")))
				|| (x == 2 && y == -2 && (direction.equals("E") || direction
						.equals("NE")))
				|| (x == 2 && y == -3 && direction.equals("SE"))
				|| (x == 3 && y == -2 && direction.equals("NW")))
			toReturn = ((PortTile) getHexTileAt(3, -3)).getPortType();

		if ((x == -3 && y == 0 && (direction.equals("SE") || direction
				.equals("E")))
				|| (x == -2 && y == 0 && (direction.equals("NW") || direction
						.equals("W")))
				|| (x == -2 && y == -1 && direction.equals("SW"))
				|| (x == -3 && y == 1 && direction.equals("NE")))
			toReturn = ((PortTile) getHexTileAt(-3, 0)).getPortType();

		if ((x == 3 && y == -1 && (direction.equals("W") || direction
				.equals("NW")))
				|| (x == 2 && y == -1 && (direction.equals("E") || direction
						.equals("SE")))
				|| (x == 3 && y == -2 && direction.equals("SW"))
				|| (x == 2 && y == 0 && direction.equals("NE")))
			toReturn = ((PortTile) getHexTileAt(3, -1)).getPortType();

		if ((x == 2 && y == 1 && (direction.equals("W") || direction
				.equals("NW")))
				|| (x == 1 && y == 1 && (direction.equals("E") || direction
						.equals("SE")))
				|| (x == 2 && y == 0 && direction.equals("SW"))
				|| (x == 1 && y == 2 && direction.equals("NE")))
			toReturn = ((PortTile) getHexTileAt(2, 1)).getPortType();

		if ((x == 0 && y == 3 && (direction.equals("NE") || direction
				.equals("NW")))
				|| (x == 0 && y == 2 && (direction.equals("SE") || direction
						.equals("SW")))
				|| (x == 1 && y == 2 && direction.equals("W"))
				|| (x == -1 && y == 3 && direction.equals("E")))
			toReturn = ((PortTile) getHexTileAt(0, 3)).getPortType();

		if ((x == -2 && y == 3 && (direction.equals("NE") || direction
				.equals("E")))
				|| (x == -1 && y == 2 && (direction.equals("W") || direction
						.equals("SW")))
				|| (x == -1 && y == 3 && direction.equals("NW"))
				|| (x == -2 && y == 2 && direction.equals("SE")))
			toReturn = ((PortTile) getHexTileAt(-2, 3)).getPortType();

		if ((x == -3 && y == 2 && (direction.equals("NE") || direction
				.equals("E")))
				|| (x == -2 && y == 1 && (direction.equals("W") || direction
						.equals("SW")))
				|| (x == -2 && y == 2 && direction.equals("NW"))
				|| (x == -3 && y == 1 && direction.equals("SE")))
	...
	/**
	 * updates the position of the robber
	 * @param x
	 * @param y
	 */
	...
	/**
	 * @param edge
	 *            , which edge we need to check
	 * @param player
	 *            , which player the edge is being checked if belonged to
	...
	/**
	 * @param corner
	 *            , which corner we need to check
	 * @param player
	 *            , which player the corner is being checked if belonged to
	 * @return boolean result
	 */
	 ...
	 * 
	 * @param hTile
	 *            the tile we are checking for neighboring edges and corners of
	 * @param edgeLoc
	 *            the current edge on the hex we are checking for neighbors of
	 * @param player
	 *            the player that we want to find a neighboring edge of
	 * @return whether this given edge has a neighboring edge or corner owned by
	 *         the given player
	 */
	public boolean hasNeighborsEdge(HexTile hTile, EdgeLocation edgeLoc,
			Player player) {
		switch (edgeLoc.getDir()) {
		case NorthWest:
			if (checkEdgeOwned(hTile.northEdge, hTile.northWestCorner, player)
					|| checkEdgeOwned(hTile.southWestEdge, hTile.westCorner,
							player)
					|| checkCornerOwned(hTile.westCorner, player)
					|| checkCornerOwned(hTile.northWestCorner, player))
				return true;
			break;
		case North:
			if (checkEdgeOwned(hTile.northEastEdge, hTile.northEastCorner,
					player)
					|| checkEdgeOwned(hTile.northWestEdge,
							hTile.northEastCorner, player)
					|| checkCornerOwned(hTile.northEastCorner, player)
					|| checkCornerOwned(hTile.northWestCorner, player))
				return true;
			break;
		case NorthEast:
			if (checkEdgeOwned(hTile.northEdge, hTile.northEastCorner, player)
					|| checkEdgeOwned(hTile.southEastEdge, hTile.eastCorner,
							player)
					|| checkCornerOwned(hTile.northEastCorner, player)
					|| checkCornerOwned(hTile.eastCorner, player))
				return true;
			break;
		case SouthEast:
			if (checkEdgeOwned(hTile.northEastEdge, hTile.eastCorner, player)
					|| checkEdgeOwned(hTile.southEdge, hTile.southEastCorner,
							player)
					|| checkCornerOwned(hTile.eastCorner, player)
					|| checkCornerOwned(hTile.southEastCorner, player))
				return true;
			break;
		case South:
			if (checkEdgeOwned(hTile.southEastEdge, hTile.southEastCorner,
					player)
					|| checkEdgeOwned(hTile.southWestEdge,
							hTile.southWestCorner, player)
					|| checkCornerOwned(hTile.southEastCorner, player)
					|| checkCornerOwned(hTile.southWestCorner, player))
				return true;
			break;
		case SouthWest:
			if (checkEdgeOwned(hTile.southEdge, hTile.southWestCorner, player)
					|| checkEdgeOwned(hTile.northWestEdge, hTile.westCorner,
							player)
					|| checkCornerOwned(hTile.southWestCorner, player)
					|| checkCornerOwned(hTile.westCorner, player))
				return true;
	 ...
	 * 
	 * @param hTile
	 *            the tile we are checking for neighboring corners along
	 * @param vertexLoc
	 *            the current corner on the hex we are checking for neighbors of
	 * @param player
	 *            the player that we want to find a neighboring corner of
	 * @return whether this given corner has a neighboring corner owned by the
	 *         given player
	 */
	public boolean hasNeighborsVertex(HexTile hTile, VertexLocation vertexLoc,
			Player player) {
	 ...
	 * 
	 * @param hTile
	 *            the tile we are checking for neighboring edges along
	 * @param vertexLoc
	 *            vertexLoc the current corner on the hex we are checking for
	 *            neighbors of
	 * @param player
	 *            the player that we want to find a neighboring edge of
	 * @return whether this given corner has a neighboring edge owned by the
	 *         given player
	 */
	public boolean vertexHasEdgeConnecting(HexTile hTile,
			VertexLocation vertexLoc, Player player) {
		switch (vertexLoc.getDir()) {
		case West:
	...
	// NEED TO BE DIFFERENT IN SETUP, SECOND ROAD HAS TO CONNECT TO SECOND
	// SETTLEMENT...I THINK
	/**
	 *
	 * @param e
	...
	public boolean canBuildRoad(Edge edge, Player player, Boolean allowDisconnected) {
		...
		if (allowDisconnected)
			...
			for (EdgeLocation edgeLoc : places) {
				HexTile hTile = getHexTileAt(edgeLoc.getHexLoc().getX(),
						edgeLoc.getHexLoc().getY());
	...
	public boolean canBuildSettlement(Corner corner, Player player,
		...
		if (!player.hasAvailableBoardPiece(PieceType.SETTLEMENT))// First check
																	// if player
																	// has
																	// available
																	// settlement
		...
		if (corner.hasStructure())
			...
			for (VertexLocation vertexLoc : places) {// Check for any neighbors
														// that prevent building
				HexTile hTile = getHexTileAt(vertexLoc.getHexLoc().getX(),
						vertexLoc.getHexLoc().getY());
				if (hasNeighborsVertex(hTile, vertexLoc, player))
					return false;
			}
			if (allowDisconnected
					|| ControllerFacade.getSingleton().getGameState() == GameState.Setup)
				return true;
			else {
				for (VertexLocation vertexLoc : places) {// Check for a road
															// connecting to
															// this corner
					HexTile hTile = getHexTileAt(vertexLoc.getHexLoc().getX(),
							vertexLoc.getHexLoc().getY());
	 ...
	 *            player to check
	 * @return
	 * @pre assumes that corner is a valid (non-null) corner (i.e. not one
	...
	public Set<Player> getPlayersOn(HexLocation hexLoc) {
		...
		if (hTile.northWestCorner.hasStructure()) {
			if (hTile.northWestCorner.getStructure().getOwner() != ControllerFacade
					.getSingleton().getClientPlayer())
				playersOn.add(hTile.northWestCorner.getStructure().getOwner());
		}
		if (hTile.northEastCorner.hasStructure()) {
			if (hTile.northEastCorner.getStructure().getOwner() != ControllerFacade
					.getSingleton().getClientPlayer())
				playersOn.add(hTile.northEastCorner.getStructure().getOwner());
		}
		if (hTile.eastCorner.hasStructure()) {
			if (hTile.eastCorner.getStructure().getOwner() != ControllerFacade
					.getSingleton().getClientPlayer())
				playersOn.add(hTile.eastCorner.getStructure().getOwner());
		}
		if (hTile.southEastCorner.hasStructure()) {
			if (hTile.southEastCorner.getStructure().getOwner() != ControllerFacade
					.getSingleton().getClientPlayer())
				playersOn.add(hTile.southEastCorner.getStructure().getOwner());
		}
		if (hTile.southWestCorner.hasStructure()) {
			if (hTile.southWestCorner.getStructure().getOwner() != ControllerFacade
					.getSingleton().getClientPlayer())
				playersOn.add(hTile.southWestCorner.getStructure().getOwner());
		}
		if (hTile.westCorner.hasStructure()) {
			if (hTile.westCorner.getStructure().getOwner() != ControllerFacade
					.getSingleton().getClientPlayer())
