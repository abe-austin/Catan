...
public class DomesticTradeController extends Controller implements
	...
	public void gameModelChanged(GameModel gameModel) {
		if (controllerFacade.getGameState() == GameState.GamePlay) {
			...
			if (gameModel.getTradeOffer().getReceiverIndex() != -1) {// there is
																		// an
																		// offer
				if (!inAcceptingTrade) {

					if (gameModel.getTradeOffer().getReceiverIndex() == controllerFacade
							.getClientPlayer().getIndex()) {
						// the offer is for me
	...
	/**
	 * set the text for the accept trade overlay
	 * checks to see if player has the resources to trade
		...
		if (!inA2) {
			...
			if (brick != 0) {
				if (brick < 0) {
					...
					getAcceptOverlay()
							.addGetResource(ResourceType.BRICK, brick);
				}
			}
			if (ore != 0) {
				if (ore < 0) {
			...
			if (sheep != 0) {
				if (sheep < 0) {
					...
					getAcceptOverlay()
							.addGetResource(ResourceType.SHEEP, sheep);
				}
			}
			if (wood != 0) {
				if (wood < 0) {
			...
			if (wheat != 0) {
				if (wheat < 0) {
					...
					getAcceptOverlay()
							.addGetResource(ResourceType.WHEAT, wheat);
				}
			}
	...
	public void sendTradeOffer() {
		for (ResourceType resource : givingResource) {
			resourceTradeAmount.put(resource,
					(resourceTradeAmount.get(resource) * -1));
		}
