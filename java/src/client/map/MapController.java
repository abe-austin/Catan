...
public class MapController extends Controller implements IMapController,
	...
	/**
	 * controls how many pieces you can place and what type of piece during the game setup state
	 */
	...
	protected void initFromModel() {
		...
		// This undoes everything I've done with the random world
		// generation...those posers.
		...
		for (int x = 0; x <= 3; ++x) {
			...
			for (int y = -3; y <= maxY; ++y) {
				...
				// getView().placeRoad(new EdgeLocation(hexLoc,
		...
		// EdgeDirection.North), portType);
		// getView().addPort(new EdgeLocation(new HexLocation(0, -3),
		// EdgeDirection.South), portType);
	...
	/**
	 * updates the map to reflect any changes
	 * @param newMap
	 */
	private void updateMap(List<HexTile> newMap) {
		...
		for (int i = 0; i < newMap.size(); i++) {
			...
			if ((x == 1 && y == -3) || (x == 3 && y == -3)
					...
					|| (x == 0 && y == 3) || (x == -2 && y == 3)
					|| (x == -3 && y == 2) || (x == -3 && y == 0)
					|| (x == -1 && y == -2)) {// See if is a port
				HexLocation hexLoc = new HexLocation(newMap.get(i).getX(),
						newMap.get(i).getY());
				PortType portType = ((PortTile) newMap.get(i)).getPortType();

				EdgeLocation edgeLocation = null;
				if ((x == 1 && y == -3) || (x == -1 && y == -2))
					edgeLocation = new EdgeLocation(new HexLocation(x, y),
							EdgeDirection.South);
				if (x == 3 && y == -3)
					edgeLocation = new EdgeLocation(new HexLocation(x, y),
							EdgeDirection.SouthWest);
			...
			HexLocation hexLoc = new HexLocation(newMap.get(i).getX(), newMap
					.get(i).getY());
			...
			if (hexType == HexType.DESERT)
				...
				getView().addNumber(new HexLocation(x, y),
						resourceTile.getToken().getValue());
			}

		}
	}
	/**
	 * updates the position of cities, settelments, or roads
	 * @param newStructures
	 */
	...
	/**
	 * set if a piece has been placed
	 */
	...
	/**
	 * places a road
	 * @param edgeLoc - location the road is built on
	 * @param color - color of the road piece
	 */
	...
	/**
	 * places a settlement on the map.
	 * @param vertLoc - location of the placed settlement
	 * @param color - color of the settlement
	 */
	 ...
	 * @param vertLoc - location of the placed city
	 * @param color - color of the placed city
	 */
	...
	/**
	 * check is the location is a port and returns what type of port
	 * @param vertLoc
	 * @return
	 */
	public int isPort(VertexLocation vertLoc) {
		...
		if (hexL.getX() == 1 && hexL.getY() == -3
				&& corner == VertexDirection.SouthEast || hexL.getX() == 1
				&& hexL.getY() == -3 && corner == VertexDirection.SouthWest
				|| hexL.getX() == 1 && hexL.getY() == -2
				&& corner == VertexDirection.NorthEast || hexL.getX() == 1
				&& hexL.getY() == -2 && corner == VertexDirection.NorthWest
				|| hexL.getX() == 2 && hexL.getY() == -3
				&& corner == VertexDirection.West || hexL.getX() == 0
				&& hexL.getY() == -2 && corner == VertexDirection.East)
			return getPortType(1, -3);

		if (hexL.getX() == 3 && hexL.getY() == -3
				&& corner == VertexDirection.West || hexL.getX() == 3
				&& hexL.getY() == -3 && corner == VertexDirection.SouthWest
				|| hexL.getX() == 2 && hexL.getY() == -2
				&& corner == VertexDirection.NorthEast || hexL.getX() == 2
				&& hexL.getY() == -2 && corner == VertexDirection.East
				|| hexL.getX() == 3 && hexL.getY() == -2
				&& corner == VertexDirection.NorthWest || hexL.getX() == 2
				&& hexL.getY() == -3 && corner == VertexDirection.SouthEast)
			return getPortType(3, -3);

		if (hexL.getX() == 2 && hexL.getY() == -2
				&& corner == VertexDirection.SouthWest || hexL.getX() == 3
				&& hexL.getY() == -1 && corner == VertexDirection.West
				|| hexL.getX() == 3 && hexL.getY() == -1
				&& corner == VertexDirection.NorthWest || hexL.getX() == 2
				&& hexL.getY() == -1 && corner == VertexDirection.SouthEast
				|| hexL.getX() == 2 && hexL.getY() == -1
				&& corner == VertexDirection.East || hexL.getX() == 2
				&& hexL.getY() == 0 && corner == VertexDirection.NorthEast)
			return getPortType(3, -1);

		if (hexL.getX() == 2 && hexL.getY() == 1
				&& corner == VertexDirection.West || hexL.getX() == 2
				&& hexL.getY() == 1 && corner == VertexDirection.NorthWest
				|| hexL.getX() == 1 && hexL.getY() == 1
				&& corner == VertexDirection.SouthEast || hexL.getX() == 1
				&& hexL.getY() == 1 && corner == VertexDirection.East
				|| hexL.getX() == 1 && hexL.getY() == 2
				&& corner == VertexDirection.NorthEast || hexL.getX() == 2
				&& hexL.getY() == 0 && corner == VertexDirection.SouthWest)
			return getPortType(2, 1);

		if (hexL.getX() == 0 && hexL.getY() == 3
				&& corner == VertexDirection.NorthWest || hexL.getX() == 0
				&& hexL.getY() == 3 && corner == VertexDirection.NorthEast
				|| hexL.getX() == 0 && hexL.getY() == 2
				&& corner == VertexDirection.SouthEast || hexL.getX() == 0
				&& hexL.getY() == 2 && corner == VertexDirection.SouthWest
				|| hexL.getX() == 1 && hexL.getY() == 2
				&& corner == VertexDirection.West || hexL.getX() == -1
				&& hexL.getY() == 3 && corner == VertexDirection.East)
			return getPortType(0, 3);

		if (hexL.getX() == -2 && hexL.getY() == 3
				&& corner == VertexDirection.East || hexL.getX() == -2
				&& hexL.getY() == 3 && corner == VertexDirection.NorthEast
				|| hexL.getX() == -1 && hexL.getY() == 2
				&& corner == VertexDirection.SouthWest || hexL.getX() == -1
				&& hexL.getY() == 2 && corner == VertexDirection.West
				|| hexL.getX() == -1 && hexL.getY() == 3
				&& corner == VertexDirection.NorthWest || hexL.getX() == -2
				&& hexL.getY() == 2 && corner == VertexDirection.SouthEast)
			return getPortType(-2, 3);

		if (hexL.getX() == -3 && hexL.getY() == 2
				&& corner == VertexDirection.East || hexL.getX() == -3
				&& hexL.getY() == 2 && corner == VertexDirection.NorthEast
				|| hexL.getX() == -2 && hexL.getY() == 1
				&& corner == VertexDirection.SouthWest || hexL.getX() == -2
				&& hexL.getY() == 1 && corner == VertexDirection.West
				|| hexL.getX() == -2 && hexL.getY() == 2
				&& corner == VertexDirection.NorthWest || hexL.getX() == -3
				&& hexL.getY() == 1 && corner == VertexDirection.SouthEast)
			return getPortType(-3, 2);

		if (hexL.getX() == -3 && hexL.getY() == 0
				&& corner == VertexDirection.East || hexL.getX() == -3
				&& hexL.getY() == 0 && corner == VertexDirection.SouthEast
				|| hexL.getX() == -2 && hexL.getY() == 0
				&& corner == VertexDirection.NorthWest || hexL.getX() == -2
				&& hexL.getY() == 0 && corner == VertexDirection.West
				|| hexL.getX() == -2 && hexL.getY() == 1
				&& corner == VertexDirection.SouthWest || hexL.getX() == -3
				&& hexL.getY() == 1 && corner == VertexDirection.NorthEast)
			return getPortType(-3, 0);

		if (hexL.getX() == -1 && hexL.getY() == -2
				&& corner == VertexDirection.SouthEast || hexL.getX() == -1
				&& hexL.getY() == -2 && corner == VertexDirection.SouthWest
				|| hexL.getX() == -1 && hexL.getY() == -1
				&& corner == VertexDirection.NorthEast || hexL.getX() == -1
	...
	/**
	 * returns port type at the location x,y
	 * @param x
	 * @param y
	 * @return
	 */
